...
class DAST(torch.nn.Module):
    def __init__(self, dim_val_s, dim_attn_s, dim_val_t,dim_attn_t,dim_val, dim_attn, time_step, 
        ...
        self.sensor_encoder = nn.ModuleList(
            [Sensors_EncoderLayer(dim_val_s, dim_attn_s, n_heads, dropout) for _ in range(n_encoder_layers)]
        )
        self.time_encoder = nn.ModuleList(
            [Time_step_EncoderLayer(dim_val_t, dim_attn_t, n_heads, dropout) for _ in range(n_encoder_layers)]
        )
        self.decoder = nn.ModuleList(
            [DecoderLayer(dim_val, dim_attn, n_heads, dropout) for _ in range(n_decoder_layers)]
        )
        ...
        self.norm1 = nn.LayerNorm(dim_val)
        self.fusion_fc = nn.Linear(dim_val, dim_val)
    ...
    def forward(self, x):
        ...
        e = self.sensor_encoder[0](self.pos_s(self.sensor_enc_input_fc(sensor_x))) #((batch_size,sensor,dim_val_s))
        ...
        # feature fusion (論文公式 11：Fr = Concat(Fs,Ft) * Wf)
        p = self.fusion_fc(self.norm1(torch.cat((e, o), dim=1)))  #((batch_size,timestep+sensor,dim_val))
