"""
Phase 7 — Generate Chinese Markdown Analysis Report
Reads stats.json accumulated by previous scripts and writes analysis_report.md
"""
import sys, os, json
sys.path.insert(0, os.path.dirname(__file__))

from utils import ANALYSIS_DIR, FIG_DIR, STATS_FILE, load_stats, RUL_CAP, N_CONDITIONS
...
def _oc_table_rows(stats):
    oc_centers = stats.get("oc_centers", {})
    rows = []
    for i in range(6):
        key = f"OC{i}_center"
        c = oc_centers.get(key, {})
        rows.append(
            f"| OC{i} | {c.get('op1','N/A')} | {c.get('op2','N/A')} "
            f"| {c.get('op3','N/A')} | {c.get('pct_train','N/A')}% |"
        )
    return "\n".join(rows)
...
# Relative paths for figures in the report
def fig(filename):
    return f"figures/{filename}"

report = f"""# CMAPSS FD004 資料集探索分析報告
...
| 操作條件數 | {N_CONDITIONS} 種 |
| 訓練集引擎數 | {s("train_units")} 台 |
| 測試集引擎數 | {s("test_units")} 台 |
| 訓練集資料行數 | {s("train_rows"):,} 行 |
| 測試集資料行數 | {s("test_rows"):,} 行 |
| 感測器數量 | 21 個 |
| 操作設定欄位 | 3 個（op1, op2, op3） |
| RUL 上限（Piecewise） | {RUL_CAP} cycles |

### 1.1 引擎壽命分佈

訓練集每台引擎的完整退化週期：

- 最短壽命：**{s("train_life_min")} cycles**
- 平均壽命：**{s("train_life_mean")} cycles**
- 最長壽命：**{s("train_life_max")} cycles**

測試集（截斷點，非完整壽命）：

- 最短記錄：**{s("test_life_min")} cycles**
- 平均記錄：**{s("test_life_mean")} cycles**
- 最長記錄：**{s("test_life_max")} cycles**

測試集真實 RUL（RUL_FD004.txt）：

- 最小 RUL：**{s("rul_true_min")} cycles**
- 平均 RUL：**{s("rul_true_mean")} cycles**
- 最大 RUL：**{s("rul_true_max")} cycles**

**圖 1-1：訓練集引擎壽命分佈**
![engine_life_dist]({fig("01_engine_life_dist.png")})

**圖 1-2：RUL 標籤分佈（Piecewise，上限 {RUL_CAP} cycles）**
![rul_dist]({fig("01_rul_distribution.png")})

**圖 1-3：測試集記錄週期分佈**
![test_life]({fig("01_test_life_dist.png")})
...
|------|-----------|------------|-----------|----------|
{_oc_table_rows(stats)}

**圖 2-1：操作條件 2D 分佈（op1 vs op2, op1 vs op3）**
![oc_scatter]({fig("02_oc_scatter_2d.png")})

**圖 2-2：操作條件 3D 分佈（★ 為聚類中心）**
![oc_3d]({fig("02_oc_3d_scatter.png")})

**圖 2-3：各操作條件出現頻率**
![oc_freq]({fig("02_oc_frequency.png")})

**圖 2-4：引擎操作條件切換時序（3 台代表引擎）**
![oc_ts]({fig("02_oc_time_series.png")})
...
- **被排除的近常數感測器（{len(s("invalid_sensors", []))} 個）**：{fmt_sensors("invalid_sensors")}
- **有效感測器（{s("n_valid_sensors", "N/A")} 個）**：{fmt_sensors("valid_sensors")}

這些被排除的感測器在整個退化過程中幾乎沒有變化，對於退化分析無貢獻。

**圖 3-1：各感測器標準差（紅色 = 被排除）**
![sensor_std]({fig("03_sensor_std.png")})
...
![sensor_ts]({fig("03_sensor_time_series.png")})

### 3.3 感測器間相關性

**圖 3-3：有效感測器 Pearson 相關矩陣**
![corr]({fig("03_sensor_correlation.png")})

### 3.4 感測器與 RUL 的關係

**圖 3-4：感測器 vs RUL 散點圖（top-6 高變異感測器）**
![svr]({fig("03_sensor_vs_rul.png")})
...
![boxplot]({fig("04_sensor_boxplot_by_oc.png")})

**圖 4-2：操作條件對感測器均值偏移熱圖（相對偏移大小）**
![offset]({fig("04_oc_offset_heatmap.png")})

### 4.2 OC 標準化的必要性

受操作條件影響最大的感測器：**{fmt_sensors("oc_shift_top3")}**

這些感測器在不同操作條件下的均值差異極大（相對偏移 CoV 最高），直接使用原始值進行退化分析會產生嚴重誤導。

**圖 4-3：OC 標準化前後的感測器–RUL 關係對比**
![norm]({fig("04_before_after_norm.png")})
...
與 RUL 相關性最高的感測器（top-6）：**{fmt_sensors("spearman_top6")}**

**圖 5-1：Spearman 相關係數（所有有效感測器，OC 標準化後）**
![spearman]({fig("05_spearman_correlation.png")})

### 5.2 降解軌跡

**圖 5-2：降解時序軌跡（cycle vs 標準化感測器，5 台引擎）**
![traj]({fig("05_degradation_trajectories.png")})

**圖 5-3：RUL 對齊降解軌跡（以剩餘壽命為 x 軸）**
![rul_traj]({fig("05_rul_aligned_trajectories.png")})

### 5.3 PCA 健康指標（Health Indicator）

將所有有效的 OC 標準化感測器進行 PCA，取第一主成分作為健康指標（HI）：

- PC1 解釋變異量：**{s("pca_pc1_var")}%**
- PC2 解釋變異量：**{s("pca_pc2_var")}%**
- HI 與 RUL 的 Spearman 相關：**ρ = {s("hi_spearman_rho")}**

**圖 5-4：PCA 健康指標（cycle 趨勢 + vs RUL）**
![hi]({fig("05_health_indicator.png")})
...
| 特徵類型 | 說明 |
|---------|------|
| 標準化感測器值 | {s("n_valid_sensors", "N/A")} 個 OC 標準化感測器 |
| 滑動視窗均值 | 前 {s("WINDOW", 30)} cycles 的滑動平均（每個感測器） |
| 滑動視窗標準差 | 前 {s("WINDOW", 30)} cycles 的滑動標準差（每個感測器） |
| 週期數（cycle） | 當前週期 |
| 操作條件（OC） | 當前操作條件類別（0–5） |
| **總特徵數** | **{s("n_features", "N/A")} 個** |

訓練樣本數：**{s("n_train_samples", "N/A"):,} 筆**
...
| 評估指標 | 數值 |
|---------|------|
| RMSE | **{s("rf_rmse")} cycles** |
| MAE | **{s("rf_mae")} cycles** |
| MAPE | **{s("rf_mape")}%** |
| PHM'08 Score | **{s("rf_phm_score")}**（越低越好） |

**圖 6-1：Random Forest 特徵重要性（top-30）**
![feat_imp]({fig("06_feature_importance.png")})

**圖 6-2：預測值 vs 真實 RUL**
![pred_actual]({fig("06_pred_vs_actual.png")})

**圖 6-3：預測誤差分佈**
![err_dist]({fig("06_error_distribution.png")})

**圖 6-4：預測誤差 vs 真實 RUL**
![err_rul]({fig("06_error_by_rul.png")})
...
| 評估指標 | 數值 |
|---------|------|
| RMSE | **{s("dast_rmse")} cycles** |
| MAE | **{s("dast_mae")} cycles** |
| MAPE | **{s("dast_mape")}%** |
| PHM'08 Score | **{s("dast_phm_score")}**（越低越好） |

**圖 7-1：DAST 預測值 vs 真實 RUL**
![dast_pred_actual]({fig("08_dast_pred_vs_actual.png")})

**圖 7-2：DAST 預測誤差分佈**
![dast_err_dist]({fig("08_dast_error_dist.png")})

**圖 7-3：DAST 預測誤差 vs 真實 RUL（熱圖顯示誤差大小）**
![dast_err_rul]({fig("08_dast_error_by_rul.png")})
...
![perm_imp]({fig("09_dast_permutation_importance.png")})
...
![saliency]({fig("09_dast_gradient_saliency_heatmap.png")})

**圖 8-3：感測器重要性 + 時間步重要性（梯度法 vs 排列法）**
![combined]({fig("09_dast_sensor_time_importance.png")})
...
![dast_vs_rf]({fig("08_dast_vs_rf_comparison.png")})

### 8.1 指標對比表

| 評估指標 | DAST | Random Forest | 優勝 |
|---------|------|---------------|------|
| RMSE（cycles） | **{s("dast_rmse")}** | {s("rf_rmse")} | {"**DAST**" if float(s("dast_rmse", 999)) < float(s("rf_rmse", 0)) else "RF"} |
| MAE（cycles） | {s("dast_mae")} | **{s("rf_mae")}** | {"DAST" if float(s("dast_mae", 999)) < float(s("rf_mae", 0)) else "**RF**"} |
| MAPE（%） | {s("dast_mape")} | **{s("rf_mape")}** | {"DAST" if float(s("dast_mape", 999)) < float(s("rf_mape", 0)) else "**RF**"} |
| PHM'08 Score | **{s("dast_phm_score")}** | {s("rf_phm_score")} | {"**DAST**" if float(s("dast_phm_score", 999)) < float(s("rf_phm_score", 0)) else "RF"} |

### 8.2 分析

1. **RMSE（均方根誤差）**：DAST 的 RMSE（{s("dast_rmse")} cycles）低於 Random Forest（{s("rf_rmse")} cycles），代表在平均預測精度上 DAST 略勝一籌。

2. **PHM'08 非對稱評分**：DAST 的 PHM Score（{s("dast_phm_score")}）遠低於 RF（{s("rf_phm_score")}），說明 DAST 更少出現「過晚預測失效」（低估 RUL）的危險錯誤。PHM Score 對於早期預警更具實用意義。
...
2. **有效感測器**：21 個感測器中有 {len(s("invalid_sensors", []))} 個為近常數（無資訊量），應予排除；其餘 {s("n_valid_sensors")} 個感測器在 OC 標準化後均展現退化趨勢。

3. **退化模式**：FD004 的 2 種故障模式（HPC + Fan Degradation）造成各引擎退化軌跡具有較大個體差異，比 FD001/FD003（單一故障模式）更難預測。

4. **PCA 健康指標**：第一主成分整合多感測器訊號，與 RUL 的 Spearman 相關 ρ = {s("hi_spearman_rho")}，是一個有效的整合健康狀態指標。

5. **DAST 模型表現最佳**：在 RMSE（{s("dast_rmse")} cycles）和 PHM'08 Score（{s("dast_phm_score")}）兩項關鍵指標上，預訓練 DAST 均優於 Random Forest 基線，特別是在安全關鍵的非對稱評分上領先顯著。
...
print("\nPhase 7 complete — Analysis report generated.")
